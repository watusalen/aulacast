...
public final class PrivilegedPortRedirectService {
    ...
    /// na fila principal quando a professora responder ao diálogo (ou ele expirar).
    public func start(targetPort: UInt16, completion: @escaping (Status) -> Void) {
        ...
        """
        runPrivileged(script) { result in
            switch result {
            case .success:
                completion(.ativo)
    ...
    /// `start` nunca chegou a rodar com sucesso nesta transmissão.
    public func stop() {
